...
YAML OpenAPI and Swagger import validated on 15 September 2026: local files and explicitly selected URLs accept JSON or YAML roots and transitively referenced documents through the existing bounded acquisition and review pipeline. YAML normalization rejects duplicate keys, multiple documents, unsupported tags, non-string mapping keys, excessive aliases/nesting, and expanded trees over 20 MiB. Eight downloaded real-world YAML documents passed Duckie's parser and importer: the supplied Microsoft WeatherData specification plus OpenAPI Initiative Petstore, Swagger Petstore, DigitalOcean, GitHub REST API, OpenAI, Plaid, and Stripe. The Microsoft OpenAPI 3.2 document produced two operations, one server, and no diagnostics; the two largest documents, GitHub and Stripe, also exercised the raised structural parsing budget while remaining inside the normalized byte limit. This was direct parser/importer validation after downloading each document, not an interactive UI walkthrough or validation of every external reference. Formatting, strict workspace Clippy, and all 139 non-measurement workspace tests passed; four performance measurements remained intentionally ignored. Both Node-backed end-to-end tests passed. The complete production release workspace and portable ZIP were rebuilt, producing `dist/Duckie-1.0.0-windows-x64.zip`. The seven newly locked dependency packages were reviewed as permissively licensed.

Bounded request history validated on 15 September 2026: direct desktop sends create chronological session-only entries with their exact editable request/test snapshot, masked effective request summary, environment, response/test outcome, duration, and shared response handle. Up to 100 metadata entries and 20 MiB of bodies are retained; oldest bodies are visibly evicted first, and a captured input can open as a new unsaved draft whose bound secret values resolve afresh from the current environment. Formatting, strict workspace Clippy, and all 135 non-measurement workspace tests passed; four performance measurements remained intentionally ignored. Both Node-backed end-to-end tests passed. A focused single-trial 50 MiB response measurement remained within the established response-memory delta; see [PERFORMANCE.md](PERFORMANCE.md#bounded-session-history-15-september-2026). This was not the full multi-trial performance protocol. Release binaries were built with the development bench feature for that measurement; production release binaries and the portable ZIP were not rebuilt or revalidated.

Request timing and connection diagnostics validated on 15 September 2026: completed responses record the negotiated HTTP version, request-start-to-response-headers duration, and combined body-transfer/decoding duration alongside the existing total. Request details explicitly label lower-level or overlapping phases and connection reuse as unavailable rather than inferring precision the transport does not expose. Formatting, strict workspace Clippy, and all 134 non-measurement workspace tests passed; four performance measurements remained intentionally ignored. Release binaries and the portable ZIP were not rebuilt or revalidated.

CLI icon and portable-package integration validated on 15 September 2026: the CLI now embeds its own multi-resolution Windows icon generated from `assets/duckie-cli.png`, and the packaging script includes `duckie-cli.exe` alongside the desktop app and test worker. A locked release build of the complete workspace and the packaging script succeeded, producing `dist/Duckie-1.0.0-windows-x64.zip`; the archive was inspected and contains all three executables. Focused strict Clippy and all three `duckie-cli` tests passed. This was not a full workspace test run.

Resolved request preview, response comparison, assertion helpers, the desktop sequential runner, and portable reports validated on 14 September 2026: formatting, strict workspace Clippy, and all 134 non-measurement workspace tests passed; four performance measurements remained intentionally ignored. Preview uses the send preparation path and retains only a masked projection with variable provenance. Comparison is bounded to 2 MiB and supports retained responses, explicit local baselines, and RFC 6901 ignore rules. Generated visible assertions cover status, headers, JSON values/types/array lengths, and duration, with path-aware bounded mismatch output. The desktop runner freezes hydrated request revisions and supports selected/folder/collection scope, stop/continue, cancellation, and summary retention. Desktop and CLI results export deterministic summary JSON, JUnit XML, and standalone HTML; response snippets require an explicit CLI option and redact known secrets. Both Node-backed end-to-end tests passed. A direct CLI smoke test ran the two-request suite, wrote and parsed JUnit XML, then ran the named request with JSON output, an HTML artifact, and an explicitly included response snippet; both commands exited successfully. Release binaries and the portable ZIP were not rebuilt or revalidated.
...
Lenient timeout defaults validated on 14 September 2026 in the working tree: new requests now default to 10 minutes, connection establishment shares that request deadline instead of having a hidden 10-second cutoff, and remote OpenAPI acquisition uses a 10-minute aggregate deadline while retaining its attempt and byte limits. `cargo fmt --all -- --check`, strict workspace Clippy, and all 121 non-measurement workspace tests passed; four performance measurements remained intentionally ignored. Both Node-backed end-to-end tests passed. Release binaries and the portable ZIP were not rebuilt or revalidated for this change.
...
- Prepared request preview shares Send validation and resolution while exposing only redacted URL, headers, body representation, environment, and variable provenance.
- Retained responses and explicit bounded baseline files can be compared by status, normalized headers, bounded text, or structural JSON with active RFC 6901 ignore rules.
- Response inspection can generate editable status, header, JSON pointer value/type/length, and duration assertions; pointer-aware failures name the mismatched path.
- The desktop sequential runner executes frozen selected, folder, or collection inputs with stop/continue and cancellation, then exports the common result model as JSON, JUnit XML, or standalone HTML.
...
- Responses use bounded streaming decompression with separate encoded/decoded caps and a 2 MiB spill threshold. Preview pages adapt to long lines; at most three responses remain attached to request panes. A chronological session history retains up to 100 direct-send metadata entries and 20 MiB of shared response bodies, evicting bodies before metadata, and can reopen a captured input as a new draft. Raw-body find scans bounded chunks off the UI thread. Progress distinguishes received and decoded bytes. Request details report total, response-header, and combined body-transfer/decoding timing plus negotiated HTTP version; phases the transport cannot reliably distinguish remain explicitly unavailable.
...
- Swagger 2.0 and OpenAPI 3.0/3.1/3.2 JSON or YAML import from local files or protected URLs into a review draft. JSON and bounded JSON-compatible YAML documents can be mixed across the root and references; ambiguous or pathological YAML is rejected before normalization. Internal, contained local and same-origin remote references resolve transitively with rebasing. External acquisition is bounded at 50 attempts and 20 MiB; URL imports also share cancellation and a 10-minute deadline. Import handles opaque examples, bounded placeholder generation, read-only omission, server precedence, security alternatives/combined requirements, `allOf` merges, and selectable `oneOf`/`anyOf` bodies.
