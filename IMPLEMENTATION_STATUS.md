...
- Swagger 2.0 and OpenAPI 3.0/3.1/3.2 JSON import from local files or protected URLs into a review draft. Internal, contained local and same-origin remote references resolve transitively with rebasing. External acquisition is bounded at 50 attempts and 20 MiB; URL imports also share cancellation and a 60-second deadline. Import handles opaque examples, bounded placeholder generation, read-only omission, server precedence, security alternatives/combined requirements, `allOf` merges, and selectable `oneOf`/`anyOf` bodies.
